...
export function ProfilesPage() {
  ...
  const create = async (e: any) => {
    ...
    setShowForm(false)
    setForm({ name: '', plan_id: 1, node_id: 1, inbound_ids: [], expires_at: '' })
    load()
  }
  ...
  const toggleInbound = (id: number) => {
    setForm((f: any) => ({
      ...f,
      inbound_ids: f.inbound_ids.includes(id) ? f.inbound_ids.filter((i: number) => i !== id) : [...f.inbound_ids, id]
    }))
  }

  const statusClass = (s: string) => {
    return (s === 'active' ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700')
  }
  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-6">
        ...
        <button onClick={() => setShowForm(!showForm)} className="bg-blue-600 text-white px-4 py-2 rounded-lg text-sm">+ Add Client</button>
      </div>
        ...
        <form onSubmit={create} className="bg-white dark:bg-neutral-800 p-4 rounded-xl shadow-sm mb-4 border">
          <div className="grid grid-cols-2 gap-3 mb-3">
            <div>
              ...
              <input value={form.name} onChange={e => setForm({...form, name: e.target.value})} required
                className="w-full border rounded-lg px-3 py-2 dark:bg-neutral-700" placeholder="Client name" />
            </div>
            <div>
              <label className="text-xs text-gray-500">Plan</label>
              <select value={form.plan_id} onChange={e => setForm({...form, plan_id: +e.target.value})}
                className="w-full border rounded-lg px-3 py-2 dark:bg-neutral-700">
                {plans.map((p: any) => <option key={p.id} value={p.id}>{p.name} ({p.price} RUB)</option>)}
              </select>
            </div>
            <div>
          ...
          <div className="mb-3">
            ...
            <div className="flex flex-wrap gap-2 mt-1">
              {inbounds.map((ib: any) => (
                <label key={ib.id}
                  className={'flex items-center gap-1 px-3 py-1.5 border rounded-lg cursor-pointer text-sm ' + (form.inbound_ids.includes(ib.id) ? 'bg-blue-50 border-blue-400' : '')}>
                  ...
                  <span className="text-xs text-gray-400">:{ib.port}</span>
                </label>
              ))}
            </div>
          </div>
          ...
          <div className="flex gap-2">
            ...
            <button type="button" onClick={() => setShowForm(false)} className="text-red-500 text-sm px-4 py-2">Cancel</button>
          </div>
        </form>
      )}
            ...
            {profiles.map((p: any) => (
              <tr key={p.id} className="border-t dark:border-neutral-700">
                ...
                <td className="px-4 py-3">
                  <span className={'px-2 py-0.5 rounded text-xs ' + statusClass(p.Status || p.status)}>
                    {p.Status || p.status}
                  </span>
                </td>
                <td className="px-4 py-3 text-xs">{new Date(p.ExpiresAt || p.expires_at).toLocaleDateString()}</td>
                <td className="px-4 py-3">
                  <button onClick={() => navigator.clipboard.writeText(p.SubscriptionURL || p.subscription_url)}
                    className="text-blue-600 text-xs hover:underline">Copy URL</button>
                </td>
